...
plot(Y, type = 'l', main = 'Single Realization of a Random Walk Process', xlab = 'Time', ylab = 'Y')
dev.off()
...
T <- 1000  # Sample size for each path
...
for (i in 1:N) {
  ...
  epsilon <- rnorm(T)
  # Cumulative sum to get the random walk path
...
matplot(t(random_walks), type = 'l', lty = 1, col = alpha('black', 0.5), ylim = c(y_min, y_max),
        xlab = 'Time', ylab = 'Y', main = '50 Independent Random Walk Paths')
dev.off()
