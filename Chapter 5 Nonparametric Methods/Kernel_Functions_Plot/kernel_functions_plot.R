...
# Create the plot using ggplot2. Colour and line type are both mapped
# so the curves remain distinguishable in black-and-white print.
p <- ggplot(df_kernels, aes(x = x, y = y, color = kernel, linetype = kernel)) +
  geom_line(size = 1.2) +
